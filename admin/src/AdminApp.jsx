...
function LabsPage() {
  ...
  function startEdit(lab) {
    setEditing(lab)
    setForm({ name: lab.name || '', address: lab.address || '', timing: lab.timing || '', rating: lab.rating || '', nabl: lab.nabl || 0, home_collection: lab.home_collection || 0 })
  }
    ...
    <div>
      ...
      {editing && (
        <Modal title={`Edit Lab: ${editing.name}`} onClose={() => setEditing(null)}>
          {[{ key: 'name', label: 'Name' }, { key: 'address', label: 'Address' }, { key: 'timing', label: 'Timing' }, { key: 'rating', label: 'Rating', type: 'number' }].map(f => (
