...
function LabsPageML({ T, catF, setCatF, setLab, setTestQ, navTo, cart, selectedTest, setSelectedTest, addCart, setCartOpen, allLabs }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F5F7FA", fontFamily:"'Manrope',sans-serif" }}>
      ...
      {/* ── PAGE HEADER ── */}
      <div style={{ background:"#1158A6", borderBottom:"1px solid rgba(255,255,255,.1)", padding:"24px 0 28px" }}>
        <div style={{ ...T.wrap }}>
          <div style={{ display:"flex" }}><button onClick={()=>navTo("home")} style={{ background:"rgba(255,255,255,.15)",border:"1.5px solid rgba(255,255,255,.4)",color:"#fff",fontWeight:700,cursor:"pointer",fontSize:".84rem",fontFamily:"'Manrope',sans-serif",padding:"8px 16px",marginBottom:18,display:"inline-flex",alignItems:"center",gap:6,borderRadius:50 }}>
          ...
          <h1 style={{ fontFamily:"'Manrope',sans-serif", fontWeight:900, fontSize:"clamp(1.6rem,3.5vw,2.2rem)", color:"#fff", marginBottom:18, letterSpacing:"-.03em" }}>All Labs</h1>
          <div style={{ position:"relative" }} ref={labSearchRef}>
            <svg style={{ position:"absolute",left:18,top:"50%",transform:"translateY(-50%)",pointerEvents:"none" }} width="18" height="18" viewBox="0 0 20 20" fill="none"><circle cx="8.5" cy="8.5" r="5.75" stroke="#9CA3AF" strokeWidth="1.7"/><path d="M13.5 13.5L17.5 17.5" stroke="#9CA3AF" strokeWidth="1.7" strokeLinecap="round"/></svg>
            <input className="labs-search-input" value={searchQ} onChange={e=>{ setSearchQ(e.target.value); setLabSugOpen(true); }} placeholder="Search labs…" style={{ paddingLeft:48,paddingRight:20,paddingTop:14,paddingBottom:14,border:"none",borderRadius:50,fontSize:"1rem",fontFamily:"'Manrope',sans-serif",outline:"none",width:"100%",boxSizing:"border-box",background:"#fff",color:"#111",boxShadow:"0 4px 20px rgba(0,0,0,.15)",transition:"box-shadow .18s" }}
              onFocus={e=>{ e.target.style.boxShadow="0 4px 24px rgba(0,0,0,.25)"; setLabSugOpen(true); }}
              onBlur={e=>{ e.target.style.boxShadow="0 4px 20px rgba(0,0,0,.15)"; }}/>
